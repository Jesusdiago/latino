...
static void cargar_libs(lat_mv *mv) {
...
static void stack_init(lat_mv *mv) {
    mv->base_frm = calloc(MIN_FRAME_SIZE, sizeof(lat_frame));
    mv->actual_frm = mv->base_frm;
    mv->size_frm = MIN_FRAME_SIZE;
    mv->final_frm = mv->base_frm + mv->size_frm - 1;
...
// Lat_INCREF(val);
                ...
                if (name->esconst) {
                    if (tmp != NULL) {
                        latC_error(
                            mv, "Intento de reasignar valor a constante '%s'",
            ...
            case CALL_FUNCTION: {
                ...
                if (nparams == FUNCION_VAR_ARGS) {
                    ...
                    if (varargs) {
                        ...
                        num_args =
                            (mv->ptrpila + mv->ptrprevio - 1) - mv->prev_args;
                    }
                ...
                if (mv->numejec >= MAX_CALL_FUNCTION) {
                    latC_error(
                        mv, "Llamadas a funciones recursivas excedido en '%s'",
                        fun->nombre);
                }
            ...
            case LOAD_ATTR: {
                ...
                if (obj->tipo == T_DIC) {
                    val = (lat_objeto *)latH_obtener(
                        latC_checar_dic(mv, obj), latC_checar_cadena(mv, attr));
                    if (val != NULL) {
                        latC_apilar(mv, val);
                        break;
                    } else {
            ...
            case STORE_SUBSCR: {
                ...
                if (obj->tipo == T_STR) {
                    ...
                    if (ipos < 0 || ipos >= (int)strlen(sobj)) {
            ...
            case ADJUST_STACK: {
                ...
                printf("%i\t", cur.a);
#endif
                while ((int)cur.a > mv->ptrpila) {
                    latC_apilar(mv, latO_nulo);
                }
                while (mv->ptrpila >= (int)(mv->ptrprevio + cur.a)) {
                    latC_desapilar(mv);
                }
            } break;
            default:
